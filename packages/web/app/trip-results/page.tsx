...
export default function TripResultsPage() {
	...
	// ローカルストレージからAPIの結果を読み込み
	useEffect(() => {
		const savedResults = localStorage.getItem("tripResults");
		if (savedResults) {
			try {
				const results = JSON.parse(savedResults);
				if (results.tripData) setTripData(results.tripData);
				if (results.hiddenSpots) setHiddenSpots(results.hiddenSpots);
				if (results.itinerary) setItinerary(results.itinerary);
			} catch (error) {
				console.error("保存された結果の読み込みに失敗:", error);
			}
		}
		setIsLoading(false);
	}, []);
		...
		return (
			<div className="min-h-screen bg-gradient-to-br from-orange-50 via-red-50 to-amber-50 flex items-center justify-center">
				<div className="text-center">
					<div className="text-lg text-gray-600">結果を読み込んでいます...</div>
				</div>
			</div>
		);
	}
	...
	if (!tripData.destination) {
		return (
			<div className="min-h-screen bg-gradient-to-br from-orange-50 via-red-50 to-amber-50 flex items-center justify-center">
				<div className="text-center">
					<Compass className="h-16 w-16 text-orange-600 mx-auto mb-4" />
					<h1 className="text-2xl font-bold text-gray-900 mb-2">旅行プランが見つかりません</h1>
					<p className="text-gray-600 mb-4">まずは旅行計画を作成してください</p>
					...
					<div className="flex items-center justify-between mb-4">
						<div>
							<h1 className="text-3xl font-bold text-gray-900 mb-2">
								{tripData.destination}の隠れ名所発見完了！
							</h1>
							<p className="text-gray-600">
								AIが厳選した{tripData.hiddenSpots}
								箇所の真の隠れ名所をご用意しました
							</p>
						</div>
										...
										<div className="md:w-1/3">
											<img
												...
												onError={(e) => {
													...
													target.src = "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=600&h=400&fit=crop&auto=format&q=80";
												}}
											/>
										</div>
								...
								<CardContent>
									<div className="space-y-4">
										{day.activities.map((activity, index) => (
											<div
												key={index}
												className="flex items-center space-x-4 p-3 border border-orange-100 rounded-lg"
											>
												<div className="text-sm font-medium text-orange-600 w-16">
													{activity.time}
												</div>
